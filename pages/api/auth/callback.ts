...
const { NEXT_PUBLIC_COGNITO_DOMAIN, NEXT_PUBLIC_USER_POOL_CLIENT_ID, NEXT_PUBLIC_APP_CLIENT_SECRET } = process.env;
...
export default async function handler(request: NextRequest) {
  try {
    ...
    const authorizationHeader = `Basic ${Buffer.from(`${NEXT_PUBLIC_USER_POOL_CLIENT_ID}:${NEXT_PUBLIC_APP_CLIENT_SECRET}`).toString("base64")}`;
